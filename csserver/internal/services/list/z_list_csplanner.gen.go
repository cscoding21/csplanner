...
// Generate Date: 2024-11-01 13:35:53.251631444 -0700 PDT m=+0.260030183
...
func (s *ListService) UpsertList(ctx context.Context, obj List) (*common.UpdateResult[List], error) {
	existingObj, _ := s.GetListByID(ctx, obj.ID)

	if existingObj == nil {
		resp, err := s.CreateList(ctx, &obj)
		return &resp, err
	}

	obj.ID = existingObj.ID
	resp, err := s.UpdateList(ctx, &obj)
	return &resp, err
}
