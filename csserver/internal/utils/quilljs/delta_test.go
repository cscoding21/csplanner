...
func TestJeph(t *testing.T) {
	jsonIn := `{"ops": [
			{"insert":{"mention":{"index":"0","denotationChar":"@","id": "user:f18z31tcxeyqkt2ubfek","value": "Anonymous"}}, "attributes":{"bold":true}},
			{"insert":" created the resource "}, 
			{"insert":"Cloud Strife", "attributes":{"bold":true}},
			{"insert":"\\n"}
	]}`

	bytes := []byte(jsonIn)
	delta, err := FromJSON(bytes)
	if err != nil {
		log.Error(err)
	}
	...
	for i, o := range delta.Ops {
		if o.InsertEmbed != nil {
			log.Infof("Op key: %v - %v", i, o.InsertEmbed.Key)
			log.Infof("Op value: %v - %v", i, o.InsertEmbed.Value)
			log.Infof("Op id: %v - %v", i, o.InsertEmbed.Value.(map[string]interface{})["id"])
		}
	}

	if len(delta.Ops) != 4 {
		t.Errorf("Incorrect number of ops, got: %d\n", len(delta.Ops))
	}
...
func TestJeph2(t *testing.T) {
	delta := New(nil).
		...
		Insert("Aerith Gainsboro ", map[string]interface{}{"bold": true})
...
// `{ "ops" :[
// 	{"insert":{"mention":{"index":"0","denotationChar":"@","id": "%s","value": "%s"}}, "attributes":{"bold":true}},
// 	{"insert":" created the resource "},
// 	{"insert":"%s", "attributes":{"bold":true}},
// 	{"insert":"\n"}
// ]}`
