<script lang="ts">
	...
	const updateCost = async () => {
		...
		costSchema.validate(projectCostParsed, { abortEarly: false })
			.then(async () => {
				await updateProjectCost(id, projectCostParsed).then(res => {
...
<p class="text-gray-500 dark:text-gray-400">Implementation cost is determined by multiplying the hourly estimates for the milestone tasks with the blended hourly rate.</p>
<div class="text-gray-50 mb-4">{formatCurrency.format(project.projectCost.initialCost as number)}</div>

<Heading tag="h6">Project Hours</Heading>
<div class="text-gray-50 mb-4">{project.projectCost.hourEstimate}</div>

<MoneyInput bind:value={costForm.ongoing as number} 
    fieldName="Ongoing Cost of Ownership (annually)" 
    error={errors.ongoing}
    update={() => update()} /> 

<MoneyInput bind:value={costForm.blendedRate as number} 
