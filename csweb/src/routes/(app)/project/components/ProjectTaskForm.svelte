<script lang="ts">
    ...
    const resetNewTask = () => {
        ...
        taskForm.milestoneID = milestoneID,
        taskForm.id = "";
        taskForm.name = "";
    ...
    const submitForm = () => {
		...
		taskSchema.validate(taskFormParsed, { abortEarly: false })
			.then(() => {
...
<div class="col-span-2">
    <TextInput bind:value={taskForm.name} 
        fieldName="Task Name" 
        error={errors.name} />
</div>
...
<div>
    <TextAreaInput bind:value={taskForm.description as string} 
        rows={2} 
