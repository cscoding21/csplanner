<script lang="ts">
	...
	const updateValue = async () => {
		...
		valueSchema.validate(projectValueParsed, { abortEarly: false })
			.then(async () => {
				await updateProjectValue(id, projectValueParsed).then(res => {
...
<PercentInput bind:value={valueForm.discountRate} 
...
<MoneyInput bind:value={valueForm.yearOneValue as number} 
    fieldName="Estimated Year One Returns" 
    error={errors.yearOneValue}
    on:updated={() => callIf(update)} /> 
  
<MoneyInput bind:value={valueForm.yearTwoValue as number} 
    fieldName="Estimated Year Two Returns" 
    error={errors.yearTwoValue}
    on:updated={() => callIf(update)} /> 
  
<MoneyInput bind:value={valueForm.yearThreeValue as number} 
    fieldName="Estimated Year Tnree Returns" 
    error={errors.yearThreeValue}
    on:updated={() => callIf(update)} /> 
  
<MoneyInput bind:value={valueForm.yearFourValue as number} 
    fieldName="Estimated Year Four Returns" 
    error={errors.yearFourValue}
    on:updated={() => callIf(update)} /> 
  
<MoneyInput bind:value={valueForm.yearFiveValue as number} 
