<script lang="ts">
	import { CSHR, CSSection, OrgStateChecker, QuillDisplay } from "$lib/components";
    ...
    <OrgStateChecker invert={false} stateToCheck="isReadyForProjects">
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <CSSection>
                ...
                {#if activity && activity.results && activity.results.length > 0}
                <ol class="divide-gray-200 mt-3 divide-y dark:divide-gray-700">
                    {#each activity.results as a, index}
                        {@render activityItem(activity.results[index] as Activity)}
                    {/each}
                </ol>
                {:else}
...
<li>
    ...
    <Avatar src={act.user.profileImage || ''} class="mr-3 h-16 w-16 min-w-16">
        {getInitialsFromName(act.user.firstName + " " + act.user.lastName)}
    </Avatar>
    <div class="text-gray-600 dark:text-gray-400">
        <QuillDisplay attachContext={act.id} bind:contents={act.detail} />
        ...
        <time datetime="2022-02-08" title={formatDateTime(act.activityDate)}>{formatDateTime(act.activityDate)}</time>
        </span>
