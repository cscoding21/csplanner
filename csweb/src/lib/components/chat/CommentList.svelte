        ...
        <div class="px-2 pt-4 mb-2 text-base bg-white rounded-lg border border-gray-200 shadow-sm dark:bg-gray-800 dark:border-gray-700 text-gray-500 dark:text-gray-400">
            ...
            {#if comment.replies && comment.replies.length > 0}
                ...
                <button class="text-xs ml-2 mb-2 flex" onclick={() => showReplies[comment.id] = false}>
                    ...
                    <span class="inline-flex cursor-pointer items-center gap-1 text-xs font-medium text-primary-700 hover:underline dark:text-primary-500 sm:text-sm">Hide {comment.replies.length} {pluralize("reply", comment.replies.length)}</span>
                </button>
                {:else}
                    <button class="text-xs ml-2 mb-2 flex" onclick={() => showReplies[comment.id] = true}>
                        ...
                        <span class="inline-flex cursor-pointer items-center gap-1 text-xs font-medium text-primary-700 hover:underline dark:text-primary-500 sm:text-sm">Show {comment.replies.length} {pluralize("reply", comment.replies.length)}
                    </span>
                    </button>
                {/if}
